 ...
 */
async function fetchServerStatus(url: string): Promise<ServerStatusResponse> {
...
function printStatus(status: ServerStatusResponse, url: string, logger: ReturnType<typeof createLogger>): void {
	console.clear()
	logger.banner()
	...
	// Server info
	logger.step(
		`Status:     Running (uptime: ${formatUptime(status.uptime)}) — v${status.version}`,
	)
