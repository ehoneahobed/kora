...
describe('ServerMetricsCollector', () => {
	it('starts with zero metrics', () => {
		...
		expect(snapshot.batchesReceived).toBe(0)
		expect(snapshot.uniqueOperationsReceived).toBe(0)
		expect(snapshot.duplicateOperationsReceived).toBe(0)
	...
	it('tracks operations received and sent', () => {
		...
		collector.recordReceived('session-1', 5, 1000, { unique: 3, duplicates: 1, rejected: 1 })
		collector.recordReceived('session-1', 3, 500)

		let snapshot = collector.getSnapshot(0)
		expect(snapshot.operationsReceived).toBe(8)
		expect(snapshot.batchesReceived).toBe(2)
		expect(snapshot.uniqueOperationsReceived).toBe(6)
		expect(snapshot.duplicateOperationsReceived).toBe(1)
	...
	it('tracks per-client metrics', () => {
		...
		expect(client!.batchesReceived).toBe(1)
