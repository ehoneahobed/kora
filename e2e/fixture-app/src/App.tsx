...
export function App() {
	...
	const { mutate: addTodo } = useMutation((data: { title: string }) => todos.insert(data))
	const { mutate: toggleTodo } = useMutation((id: string, data: { completed: boolean }) =>
		todos.update(id, data),
	)
	const { mutate: deleteTodo } = useMutation((id: string) => todos.delete(id))
