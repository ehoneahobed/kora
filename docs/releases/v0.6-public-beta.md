...
**Builds on:** [v0.5 internal beta](./v0.5-internal-beta.md) (convergence + `ApplyPipeline`). Code for Phases 2–3.2 from the best-in-class plan (retired) is already merged; v0.6 is **sign-off and DX**, not greenfield implementation.
...
| Storage | Tab leader + BroadcastChannel, OPFS `dbName`, durable IndexedDB fallback, IDB coalescing + quota events |
...
- Keep templates on the dedicated-worker leader/follower path; SharedWorker-hosted SQLite is not durable.
